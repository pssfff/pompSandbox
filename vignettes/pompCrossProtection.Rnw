...
<<fourStrainProcSim>>=
fourStrainProcSim <- function(x, t, params, delta.t, ...) {
        ## unpack the params vector:
        beta <- params["beta"]
        alpha1 <- params["alpha1"]
        alpha2 <- params["alpha2"]
        ...
        ## udpate counts
        newx[namesS] <- x[namesS] + N*mu - newx[namesI] + NS
        newx[namesC] <- x[namesC] - NS + newx[namesI]
        return(newx[c(namesS, namesI, namesC)])
}

fourStrainMeasSim <- function(x, t, params, ...) {
...
And now we can simulate this framework
<<simulateFourStrainTSIR, cache=TRUE>>=
pomp(
        data=data.frame(
                time=seq(0, 5000,by=1),
...
simulate(
        ...
        params=c(
                ...
                beta=1.1,alpha1=1,alpha2=1,
                iota=1/10000,
                lambda=1/10000,
                S1.0=4000,I1.0=100,C1.0=100,
                S2.0=10000,I2.0=100,C2.0=100,
                S3.0=400,I3.0=100,C3.0=100,
                S4.0=40000,I4.0=100,C4.0=100
        ),
        seed=677573454L
) -> tsir
plot(tsir, variables=c("I1","I2", "I3", "I4"))
@
\end{document}